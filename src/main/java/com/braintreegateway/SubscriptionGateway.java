...
/**
 * Provides methods to interact with {@link Subscription Subscriptions}. Including create, find,
 * update, cancel, etc. This class does not need to be instantiated directly. Instead, use {@link
 * BraintreeGateway#subscription()} to get an instance of this class:
 *
 * <pre>
 ...
 * </pre>
 *
 * For more detailed information on {@link Subscription Subscriptions}, see <a
 * href="https://developers.braintreepayments.com/reference/response/subscription/java"
 * target="_blank">https://developers.braintreepayments.com/reference/response/subscription/java</a>
 */
public class SubscriptionGateway {
  ...
  public Result<Subscription> cancel(String id) {
    NodeWrapper node =
        http.put(configuration.getMerchantPath() + "/subscriptions/" + id + "/cancel");
   ...
   * @return the {@link Subscription} or raises a {@link
  ...
  public ResourceCollection<Subscription> search(SubscriptionSearchRequest searchRequest) {
    NodeWrapper node =
        http.post(
            configuration.getMerchantPath() + "/subscriptions/advanced_search_ids", searchRequest);
  ...
  List<Subscription> fetchSubscriptions(SubscriptionSearchRequest search, List<String> ids) {
    ...
    NodeWrapper response =
        http.post(configuration.getMerchantPath() + "/subscriptions/advanced_search", search);
  ...
  public Result<Transaction> retryCharge(String subscriptionId) {
    return retryCharge(new SubscriptionTransactionRequest().subscriptionId(subscriptionId));
  }

  public Result<Transaction> retryCharge(String subscriptionId, BigDecimal amount) {
    return retryCharge(
        new SubscriptionTransactionRequest().subscriptionId(subscriptionId).amount(amount));
  }

  public Result<Transaction> retryCharge(String subscriptionId, Boolean submitForSettlement) {
    return retryCharge(
        new SubscriptionTransactionRequest()
  ...
  public Result<Transaction> retryCharge(
      String subscriptionId, BigDecimal amount, Boolean submitForSettlement) {
    return retryCharge(
        new SubscriptionTransactionRequest()
            .subscriptionId(subscriptionId)
            .amount(amount)
