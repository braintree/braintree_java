    ...
    this.insights =
        new CustomerInsights()
            .paymentRecommendations(
                getPaymentRecommendations((Map<String, Object>) customerInsights.get("insights")));
  }

  private static List<PaymentRecommendation> getPaymentRecommendations(
      Map<String, Object> insightObj) {
    List<Map<String, Object>> recommendationObjs =
        (List<Map<String, Object>>) insightObj.get("paymentRecommendations");

    List<PaymentRecommendation> paymentRecommendations =
        recommendationObjs.stream()
            .map(
                recommendationObj -> {
                  Integer recommendedPriority =
                      (Integer) recommendationObj.get("recommendedPriority");
                  InsightPaymentOption paymentOption =
                      InsightPaymentOption.valueOf((String) recommendationObj.get("paymentOption"));
